...
# Filter data by rater type and selecting relevant columns for each group
# Patient
...
# Calculate percentage of patients with each symptom reported by nurses 
perc_tot_n <- c()
for (i in 3:8) {
  count_n <- 0
  for (j in 1:97) {
...
# Calculate percentage of patients with each symptom reported by patients
perc_tot_p <- c()
for (i in 3:8) {
  count_p <- 0
  for (j in 1:97) {
...
# Calculate percentage of patients with each symptom reported by doctors
perc_tot_d <- c()
for (i in 3:8) {
  count_d <- 0
  for (j in 1:97) {
...
# Create data frames for each survey method and label them
...
# Bar plot for symptom prevalence as reported by each method
