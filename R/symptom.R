...
# Load symptom data and select relevant columns
...
# Get number of symptom columns
num_symp <- ncol(data)

# Loop through each symptom column and count number of symptoms if reported 
# and percentage of patients reporting
perc_tot <- c()
for (j in 2:num_symp) {
  count <- 0
  for(i in 1:97) {
